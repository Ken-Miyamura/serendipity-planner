...
| 目的地バナー | DestinationBannerView | 目的地の未設定/設定済み/コンパクトの3状態を表示。タップで目的地検索シートを開く |
| 受け入れ済み | AcceptedCardView リスト | 「今日の予定」セクション |
| セクション見出し | 「○○で過ごす、すきま時間」/「今いる場所から、すきま時間に」 | 目的地の有無で切り替え。未設定時は「現在地・○○」チップを表示 |
| 提案リスト | FreeTimeCardView リスト | 各隙間時間スロットの提案カード |

### DestinationBannerView（目的地バナー）

| 状態 | 表示内容 |
|------|---------|
| 未設定 | 珊瑚色のピンアイコン + 「目的地を決める」+ 説明文 + ＋ボタン（点線枠のカード） |
| 設定済み（フル） | 「目的地」ラベル（緑）+ 目的地名 + 「○○周辺・N スポットを提案中」+ 変更ボタン |
| 設定済み（コンパクト） | 予定を受け入れた後などの省スペース時に 1 行表示（目的地名 + 変更ボタン） |
...
| セクション | 内容 |
|-----------|------|
| 背景 | SkyGradientView（時間帯×天気で変化。取得済みの天気条件を反映し、夜間は明色テキストに切替） |
| 出所バッジ | 「○○ 周辺から提案中」/「現在地周辺から提案中」（提案の検索基点を明示） |
| ヘッダー | カテゴリアイコン + タイトル + 時間帯/所要時間 |
| 説明 | 提案の詳細テキスト |
| スポット情報 | スポット名・距離・徒歩時間・地図ボタン（スポットがある場合）。距離表記は基点付き（「○○から徒歩 N 分 ・ X m」） |
| 地図 | MapKit による周辺地図とカスタムピン（タップでマップアプリ選択、スポットがある場合） |
| 天気コンテキスト | 天気状況とアドバイステキスト |
| アクションボタン | 「この提案を受け入れる」 |
| 受け入れ完了 | チェックマークアニメーション |
| 代替案 | 現在のカテゴリを除いた選択中カテゴリぶんの提案リスト。目的地設定時は見出しが「○○の ほかの候補」+「すべて○○エリアから選んでいます」に変化 |
...
- ピンの色はカテゴリカラーに連動
- 地図プレビュー全体がタップ可能（右上に「マップで開く」ヒントを overlay 表示）

### マップアプリ選択ダイアログ

地図ボタンまたはプレビュー地図タップ時に、下から出るボトムシート（`MapAppPickerSheet`）を表示:

- Apple マップ（常時表示）
- Google マップ（インストール済みの場合のみ）
- ブラウザで開く（常時表示）
- キャンセル

選択したアプリは徒歩経路で開きます。目的地を設定している場合は「目的地 → 提案スポット」、設定していない場合は「現在地 → 提案スポット」になります。
...
- 現在のカテゴリを除外し、**設定で選択中のカテゴリすべて**について 1 件ずつ提案を表示する（件数の上限は設けない）
  - 全 10 ジャンルを選択している場合は 9 件になる
- 各代替案をタップすると詳細画面に遷移

---

## 目的地検索シート（DestinationSearchView）

ホーム画面の目的地バナーをタップすると `sheet` で表示されます。

### 構成

| セクション | 内容 |
|-----------|------|
| ナビゲーションバー | タイトル「目的地を選ぶ」+ ×（閉じる）ボタン（グレーの丸） |
| 検索フィールド | 「エリア・駅・スポットを検索」（暖色グレー背景、`MKLocalSearchCompleter` による候補列挙。IME 変換中は取得しない） |
| 現在地を使う | 珊瑚色アイコンの行。タップで目的地を解除し現在地ベースに戻す |
| 最近の検索 | 横スクロールのチップ（時計アイコン付き、最大 6 件） |
| この近くのおすすめ | 現在地周辺から動的取得したエリア（タイル枠アイコン + 名前 + 現在地からの距離） |
| 検索候補 | 入力時に候補を一覧表示（候補名 + 住所）。該当なし・検索中の状態表示あり |

### 動作

- 検索候補・最近の検索・おすすめのいずれかを選択すると目的地を設定し、シートを閉じてホームで提案を再生成
- 検索候補は座標を持たないため、選択時に座標を解決してから確定する。解決中は「場所を確認しています...」の
  オーバーレイを表示し、候補のタップを無効化する。解決に失敗した場合はアラートを表示する
- 解決を待っている間にシートが閉じられた場合は目的地を確定しない
- IME 変換中は候補を取得せず、「該当する場所が見つかりませんでした」も表示しない（確定を待つ）
- 現在地の取得はシート表示時に能動的に行い、その位置でおすすめエリアと検索バイアスを構築
...
| セクション | 内容 |
|-----------|------|
| 背景 | SkyGradientView（提案詳細と共通。時間帯で変化し、夜間は明色テキストに切替） |
| ヘッダー | カテゴリアイコン + タイトル + カテゴリバッジ |
| 説明 | 提案の詳細テキスト |
| スポット情報 | スポット名・住所・地図ボタン（スポットがある場合） |
| 地図 | MapKit による周辺地図とカスタムピン（タップでマップアプリ選択、座標がある場合） |
| 追加日 | お気に入り登録日の表示 |
| 削除ボタン | 確認ダイアログ付きの削除ボタン |

地図プレビューと地図ボタンの両方でマップアプリ選択ダイアログ（Apple マップ / Google マップ / ブラウザで開く）を起動できます。お気に入りは過去に保存したものなので、「今日の目的地」ではなく現在地を出発点とした徒歩経路で開きます。
...
時間帯と天気条件に基づき、SkyGradientView を背景に使う画面（ホーム・履歴・お気に入り・ウィジェット）のテキストカラーを明色/暗色で切り替えます。
...
## ウィジェット（SmallWidgetView / MediumWidgetView）

### Small ウィジェット

| 領域 | コンポーネント | 説明 |
|------|-------------|------|
| 背景 | SkyColorPalette グラデーション | 時間帯に基づくグラデーション（containerBackground で全面拡張） |
| アイコン | カテゴリアイコン | カテゴリ固有の色で表示 |
| テキスト | カテゴリ名 + 時間範囲 | 時間帯に応じた明色/暗色テキスト |

### Medium ウィジェット

| 領域 | コンポーネント | 説明 |
|------|-------------|------|
| 背景 | SkyColorPalette グラデーション | 時間帯に基づくグラデーション |
| 左カラム | カテゴリアイコン（大） | カテゴリ固有の色で表示 |
| 右カラム | カテゴリ名 + 提案タイトル + 時間 + 場所 + 天気 | 階層的なテキストカラー（primary / secondary / tertiary） |

### 空き時間なし状態

- **Small**: カレンダーアイコン + 「空き時間なし」
- **Medium**: カレンダーアイコン + 「今日の空き時間はありません」+ 説明テキスト
