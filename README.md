...
- **目的地レコメンド** - 今日の目的地を設定すると、現在地ではなくその街の周辺から提案（当日限定・現在地ベースのおすすめエリア/検索付き）
...
| ホーム | 今日の隙間時間と提案一覧、天気バッジ、目的地バナー表示 |
| 目的地検索 | 今日の目的地をエリア・駅・スポットで検索、最近の検索・現在地周辺のおすすめから選択 |
| 履歴 | 受け入れた提案の月別振り返り、カテゴリ別サマリー |
| お気に入り | 気に入った提案の保存・カテゴリフィルタ |
| 提案詳細 | 提案の詳細、提案の出所（現在地/目的地）、周辺マップ、受け入れ/別の提案 |
...
├── Models/        # データモデル（FreeTimeSlot, Suggestion, WeatherData, TodayDestination等）
